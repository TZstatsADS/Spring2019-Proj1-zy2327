...
This part is grouping words into 10 segments: anger, anticipation, disgust, fear, joy, neagtive, positive, sadness, suprise and trust. Instead of measuring how strong the words are, it tries to find out what feelings different words express.
...
#ggplot()+
#  geom_col(data=s_c,mapping=aes(x=sentiment,y=N))+
...
library(gridExtra)
library(lattice)
grid.arrange(g1.fun(1,person),g1.fun(2,person),g1.fun(3,person),g1.fun(4,person),g1.fun(5,person), nrow = 3)
```
...
grid.arrange(g1.fun(1,verb),g1.fun(2,verb),g1.fun(3,verb),g1.fun(4,verb),g1.fun(5,verb), nrow = 3)
```
...
grid.arrange(g1.fun(1,object),g1.fun(2,object),g1.fun(3,object),g1.fun(4,object),g1.fun(5,object), nrow = 3)
```
...
Finally, let's take a look at the bigram they use.
...
setDT(hm_bigrams)
hm_bigrams <- transform(hm_bigrams, age = as.numeric(age))
hm_bigrams <- hm_bigrams[! is.na(hm_bigrams$age)]

g2.fun <- function(i)
{
  age.lower <- c(10,20,30,40,50)[i]
  age.upper <- c(20,30,40,50,100)[i]
  temp <- hm_bigrams[age >age.lower & age <=age.upper,.N,by = .(bigram)][order(-N)][1:10]
  temp$bigram <- factor(temp$bigram,levels=temp$bigram[seq(10,1,-1)])
  t <- paste(age.lower,"-",age.upper)
  color = c("#ADD8E6","#87CEEB","#6495ED","#4169E1","#0000CD","#000080")
  g <- ggplot()+
  geom_bar(data=data.frame(temp),mapping=aes(bigram,N),stat = "identity",fill=color[i])+
  ggtitle(t)+
  coord_flip()
  return(g)
}

grid.arrange(g2.fun(1),g2.fun(2),g2.fun(3),g2.fun(4),g2.fun(5), nrow = 3)
```
...
From above plot, we can see that work is the top mentioned word by male. "Father" tend to mentioned more "work" that non-parent males. This may due to the resposibilty to support the family.
